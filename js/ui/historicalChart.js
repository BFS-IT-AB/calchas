...
class HistoricalChart {
  ...
  render(historicalData, type = "temperature", fahrenheit = false) {
    ...
    this.data = historicalData.map((d) => ({
      ...
      temp_avg: fahrenheit
        ? (((d.temp_min + d.temp_max) / 2) * 9) / 5 + 32
  ...
  _drawTemperatureChart(fahrenheit) {
    ...
    for (let i = 0; i < this.data.length; i++) {
      ...
      const normalizedTemp =
        (this.data[i].temp_max - minTemp) / (maxTemp - minTemp);
    ...
    for (let i = this.data.length - 1; i >= 0; i--) {
      const x = padding + i * xStep;
      const normalizedTemp =
        (this.data[i].temp_min - minTemp) / (maxTemp - minTemp);
      const y = this.height - padding - normalizedTemp * chartHeight;
      this.ctx.lineTo(x, y);
    }
    ...
    for (let i = 0; i < this.data.length; i++) {
      const x = padding + i * xStep;
      const normalizedTemp =
        (this.data[i].temp_avg - minTemp) / (maxTemp - minTemp);
      const y = this.height - padding - normalizedTemp * chartHeight;
      i === 0 ? this.ctx.moveTo(x, y) : this.ctx.lineTo(x, y);
    }
    ...
    for (
      let i = 0;
      i < this.data.length;
      i += Math.ceil(this.data.length / 5)
    ) {
      const x = padding + i * xStep;
  ...
  _drawLegend(type, fahrenheit) {
    ...
    if (type === "temperature" || type === "both") {
      ...
      this.ctx.fillText(
        `Durchschn. Temperatur (°${fahrenheit ? "F" : "C"})`,
        this.width - 180,
        legendY + 4,
      );
    }
