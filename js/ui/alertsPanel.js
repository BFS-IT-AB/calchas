...
class AlertsPanel {
  ...
  async fetchAlerts(latitude, longitude) {
    try {
      ...
      try {
        ...
        console.warn(
          "⚠️ MeteoAlarm CORS restriction - alerts unavailable:",
          corsErr.message,
        );
        this.alerts = [];
      }
  ...
  _parseCAP(xmlText, latitude, longitude) {
    try {
      ...
      for (let entry of entries) {
        const title =
          entry.querySelector("title")?.textContent || "Unknown Alert";
        const summary =
          entry.querySelector("summary")?.textContent || "No description";
        ...
        const area =
          entry.querySelector("georss\\:point") || entry.querySelector("point");
  ...
  hasCriticalAlerts() {
    return this.alerts.some((alert) =>
      ["Red", "Orange"].includes(alert.severity),
    );
  }
}
